...
import { RotateCcw, ChevronLeft, ChevronRight, Check, Sun, Moon } from "lucide-react";
...
type TabMode = "counter" | "morning" | "evening";
...
const Dhikr = () => {
  ...
  const azkarList = activeTab === "morning" ? morningAzkar : eveningAzkar;
        ...
        <div className="flex-1">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-4">
            <h1 className="text-xl font-bold text-foreground flex items-center gap-2">
              {activeTab === "morning" ? <Sun size={18} className="text-accent" /> : <Moon size={18} className="text-accent" />}
              {activeTab === "morning" ? "Morning Azkar" : "Evening Azkar"}
            </h1>
            <p className="text-xs text-muted-foreground mt-0.5">
              {azkarList.length} duas · {activeTab === "morning" ? "أذكار الصباح" : "أذكار المساء"}
            </p>
          </motion.div>
